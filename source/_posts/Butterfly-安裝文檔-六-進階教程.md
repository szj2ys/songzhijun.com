...
1. 不要把個人需要的文件/圖片放在主题`source`文件夾裏，因為在升級主题的過程中，可能會把文件覆蓋刪除了。
...
主题已於 `3.4.0` 去除 jQuery 的引用，但是部分功能仍需要加載 jQuery（*Justified Gallery*和 *fancybox*)。

如果你仍需要使用 jQuery，可以調用主题提供的 function，防止 jQuery 多次加載。
...
由於Butterfly主题中的JS使用到了部分ES6語法，因此不能只使用 [gulp-uglify ](https://github.com/terinjokes/gulp-uglify)來壓縮，還需要搭配其它的插件。兩種方法都可以有效的壓縮JS代碼，選一種適合自己的就行。
...
Butterfly主题內置了[Font Awesome V5 ](https://fontawesome.com/)圖標，目前已更新到 5.13.0，總共有1,588個免費圖標。由於是國外的圖標網站，對於國內的一些網站Icon並不支持。如有需要，你可以引入其它的圖標服務商。
...
打開`主题配置文件`，找到`inject`配置，按要求把鏈接填入
...
Butterfly主题需要使用到很多圖片。如果圖片太大，會嚴重拖慢網站的加載速度。
